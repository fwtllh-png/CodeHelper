...
func TestNilSetAcceptsObservationsAndReportsNothing(t *testing.T) {
	var set *Set
	set.BeginTurn(1)
	set.Observe(Fact{Kind: KindDefinition, Path: "a.go"})
...
func TestObserveDeduplicatesAndKeepsEarliestTurn(t *testing.T) {
	set := New()
	set.BeginTurn(1)
	set.Observe(Fact{Kind: KindDefinition, Path: "a.go", Line: 12, Turn: 3, Tool: "search_symbol"})
	set.Observe(Fact{
...
func TestObserveRejectsUnknownKindAndEmptyPath(t *testing.T) {
	set := New()
	set.Observe(Fact{Kind: Kind("guess"), Path: "a.go", Turn: 1})
	set.Observe(Fact{Kind: KindDefinition, Path: "   ", Turn: 1})
...
func TestSnapshotOrdersFactsByKindThenPath(t *testing.T) {
	set := New()
	set.Observe(Fact{Kind: KindTextMatch, Path: "z.go", Turn: 1})
	set.Observe(Fact{Kind: KindReference, Path: "b.go", Turn: 1})
	set.Observe(Fact{Kind: KindDefinition, Path: "c.go", Turn: 1})
	set.Observe(Fact{Kind: KindDefinition, Path: "a.go", Turn: 1})
	snapshot := set.Snapshot(0)
	var got []string
...
func TestSnapshotLimitKeepsRecentFactsAndReportsTheRest(t *testing.T) {
	set := New()
	set.Observe(Fact{Kind: KindTextMatch, Path: "old.go", Turn: 1})
	set.Observe(Fact{Kind: KindTextMatch, Path: "new.go", Turn: 4})
...
func TestCloneIsIndependent(t *testing.T) {
	...
	clone := set.Clone()
	set.MarkVerified([]string{"a.go"})
	set.Observe(Fact{Kind: KindReference, Path: "b.go", Turn: 2})
