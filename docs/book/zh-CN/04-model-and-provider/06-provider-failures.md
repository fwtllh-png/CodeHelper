...
- 429 使用公开 Rate Limit Recovery Budget：`execution.rate_limit_wait` 默认
  `10m`（与连接阶段 `execution.timeout` 分开，`0` 仍继承 timeout），
  `execution.rate_limit_retry_limit` 默认不限次数。耗尽后转为可恢复 Blocked，
  不无限透明等待。429 恢复不消耗
