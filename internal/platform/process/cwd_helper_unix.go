...
const cwdHelperArgument = "__codehelper_internal_fchdir_exec_v1"
...
func init() {
	...
	if err := unix.Fchdir(3); err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "codehelper cwd helper: fchdir: %v\n", err)
		os.Exit(126)
	}
	path, err := exec.LookPath(os.Args[2])
	if err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "codehelper cwd helper: executable: %v\n", err)
		os.Exit(126)
	}
	arguments := append([]string{os.Args[2]}, os.Args[3:]...)
	if err := syscall.Exec(path, arguments, os.Environ()); err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "codehelper cwd helper: exec: %v\n", err)
		os.Exit(126)
	}
}
