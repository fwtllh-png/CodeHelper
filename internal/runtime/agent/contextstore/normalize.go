...
func NormalizePairs(
	...
	normalized, receipt, err := New(Input{
		History: messages,
...
// immutable Snapshot used for both attribution and provider projection.
func (s Snapshot) Normalize(
	capabilities model.Capabilities,
) (Snapshot, NormalizationReceipt, error) {
	...
	for itemIndex, item := range s.items {
		if !validRole(item.Message.Role) {
			return Snapshot{}, NormalizationReceipt{}, fmt.Errorf(
...
func visibleOrphanCount(items []Item) int {
