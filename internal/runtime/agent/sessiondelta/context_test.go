...
func TestWorkspaceReconciliationRewritesStaleTruthInHistory(t *testing.T) {
	...
	truth.Entities = []compact.TruthEntity{entity}
	truth.Seal()
	rendered, err := compact.RenderStructured(
		compact.Summary{Window: 2},
		truth,
		compact.Narrative{Lines: []string{"old workspace conclusion"}},
		0,
	)
	if err != nil {
		t.Fatal(err)
	...
	snapshot.Evidence.Changes = []evidence.Change{{
	...
	restoredTruth, found, err := compact.ParseTruthCapsule(
	...
	if got.Verified || got.WorkspaceClaimStatus != compact.WorkspaceClaimStale ||
