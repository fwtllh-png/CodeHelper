...
[English](../en/session-context-optimization.md) | 简体中文
...
4. 用户 Memory 是整文件注入，缺少 Scope、检索、更新、删除和同 Runtime 刷新。

结构化事实、语义 Narrative、用户 Memory 和 Checkpoint 必须继续拥有不同的 Authority、
Retention 和 Failure Policy，不能再次合并成一个不透明文本字段。
...
- 在任意支持的模型 Context Window 内给权威事实一个可证明的硬上界；
- 保留结构化事实的确定性、可恢复性和审计能力；
- 用可选 Narrative 提升设计讨论和长期任务的语义连续性；
- Checkpoint Restore/Fork 恢复同一时点的完整模型上下文状态；
- 将稳定 Session 的持久化写入从重复全量快照改为有界增量；
...
3. **Current state, not endless union**：当前 Owner 重新声明状态；历史代际不能永久并集。
4. **One Runtime authority**：Host 只提交 Operation 和投影 Receipt。
5. **Commit before apply**：持久化成功前不修改 Session 权威内存。
6. **Optional work cannot rewrite business outcome**：Narrative 失败只能成为维护 Receipt。
7. **State restore is not side-effect replay**：Context 与 Workspace Side Effect 明确分离。
8. **Qualification and discovery are separate**：确定性门禁证明契约，探索性评测寻找未知
   退化，不能互相冒充。
...
Authority Equivalence 只要求 `mandatory` Entity 精确等价。`protected` 和 `refreshable`
必须满足 Retention Policy，并在 Receipt 中记录保留、降级和淘汰数量。这样既不伪造
“全部事实仍在”，也不让可重新发现的事实阻塞整个 Session。

### 7.2 确定性 Retention Planner
...
`truth_max_bytes` 必须小于 `summary_max_bytes`，为 Wrapper、Omission 和 Narrative 留出空间。
Config Validation 应拒绝不可能满足的组合。
...
建议 Truth 使用 `system` Message，Narrative 使用带
`provenance=context_narrative` 的纯文本 `assistant` Message。Stable Prefix 必须明确
说明 Narrative 不是指令、权限或验证证据。Tail 保留原始 Role 和 Content Block。
...
`purpose=context_compaction` 是 Sample Attribution，不新增可绕过 Router 的 Provider
路径。初始实现复用当前 Turn 已冻结的 Route；以后只有存在明确质量证据时才增加独立
Route 配置。
...
4. `ContextCompactionPrepared` 持久化 Plan Digest 和确定性回退结果；
...
Persistent Runtime 在一个 Ownership Boundary 内提交：
...
## 9. 完整 Context Checkpoint

### 9.1 拆分 Context 与 Accounting

当前 Session Delta 同时携带 Context 和累计 Usage/Cost。优化后拆成：
...
Accounting。Restore 不能通过回退 Usage/Cost 绕过 Budget。
...
4. 替换 History、Working Set、Evidence、Failures 和 Plan；
5. 校验 World Baseline，不匹配时清空并在下一 Sample Full Project；
6. 创建新的 Token Window，不能复用旧 Provider Observation；
7. 持久化 Restore Fact、Context Manifest 和 Outbox；
8. 事务成功后才替换 Engine 内存；
9. 永不执行历史 Tool，也不修改 Workspace。

### 9.3 Fork 语义

Checkpoint Fork 必须从 Checkpoint 的 Context Snapshot 构造新 Engine，不能先复制当前
Parent Engine。新 Thread 获得：

- 独立 State Epoch 和 Window ID；
- Checkpoint 时点的 Working Set、Evidence、Failures 和 Plan；
...
事务失败时释放暂存 Ref。启动恢复只接受 Digest、Epoch 和 Revision 链全部有效的最新
Manifest。Event Reconstruction 保留为审计和灾难恢复路径，但不应在正常启动时重复构造
全部账本。
...
| CAS Ref、Manifest、Checkpoint Repository | `internal/persist` | Ref 与 SQLite 状态必须原子或可补偿 |
...
- 在 `internal/runtime/agent/compact` 增加 Retention Class 和 Planner；
- 各账本实现当前 Entity Snapshot，不再永久并集；
- 增加 CAS Handle 外置和 Omission Receipt；
- 保持 Semantic Narrative 关闭。

退出条件：480 Turn 下 Capsule 始终满足上界；Mandatory Entity 零丢失；同输入 Digest
完全一致。

### Phase 2：Context-consistent Checkpoint

- 从 `sessiondelta` 提取 `ContextSnapshot` 与 `AccountingDelta`；
- Checkpoint 保存完整 Context Snapshot；
- Restore/Fork 引入 State Epoch 和原子 Commit；
- 增加旧 History 与新 Evidence 混合的回归测试。

退出条件：Restore/Fork 后所有 Context Ledger Digest 与 Checkpoint 一致，Usage/Cost
保持单调，Side Effect Replay 始终为零。

### Phase 3：Context Manifest 与增量 History

- 定义 Manifest、History Base 和 Turn Tail Segment；
- 使用 CAS Reference 替换重复 Snapshot Payload；
- 实现 Manifest Recovery、Ref Cleanup 和 Crash Point；
- 在明确需要前不增加 Legacy 双写。

退出条件：480 Turn Physical Write Amplification 相对基线下降至少 70%；任意提交 Crash
Point 后恢复到旧 Revision 或新 Revision，不能出现混合状态。

### Phase 4：可选 Semantic Narrative

- 定义 `CompactedContext`、`CompactionPlan`、Narrative Item 和 Artifact Codec；
- 为 Turn Kernel 增加 `EffectGenerateNarrative`、`EffectCommitContextRebase` 及对应
  Command/Domain Fact；
- 通过 Provider Router 执行无 Tool、低预算模型调用；
- 实现 `post_turn` Job、`inline` Pause/Rebase、Source ID Validator、Staleness Fence
  和确定性降级；
...
任何 Narrative Claim 都不能改变 Authority Digest；Mid-turn Rebase Crash 后恢复不能
重复提交或回到旧 Window。

### Phase 5：用户 Memory v2
...
2. Bounded Truth 先以 Shadow Receipt 计算新旧结果，不改变实际 Context；
3. Checkpoint v2 在专用 Format Version 下启用；
4. Manifest 先双读单写仅限明确迁移窗口，稳定后删除旧读取路径；
...
1. 480 Turn Session 中 Truth Capsule、Model Context 和物理持久化增长均有明确上界；
2. 所有 Mandatory Entity 在重复压缩、重启、Model Downshift 后保持 Authority 等价；
3. Checkpoint Restore/Fork 不混入 Checkpoint 之后的 Context State；
4. Usage、Cost、Permission 和 Workspace Side Effect 不可通过 Restore 回退；
5. Narrative 关闭时保持确定性；开启时任何故障都不改变业务终态；
6. `inline` 模式可以暂停、原子 Rebase 并继续原 Turn，Cancel/Restart 不产生重复
   Compaction；
