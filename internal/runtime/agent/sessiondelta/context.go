...
func CloneContextSnapshot(snapshot ContextSnapshot) ContextSnapshot {
	snapshot.History = contextstore.CloneMessages(snapshot.History)
	...
	snapshot.World = contextstore.CloneWorldBaseline(snapshot.World)
	snapshot.Compaction = CloneCompaction(snapshot.Compaction)
	snapshot.Workspace.BoundPaths = append(
		[]BoundPath(nil),
		snapshot.Workspace.BoundPaths...,
	)
	snapshot.Window = contextstore.CloneWindowLedger(snapshot.Window)
	return snapshot
}
...
func ReconcileWorkspace(
	...
	snapshot.World = contextstore.WorldBaseline{}
	snapshot.Epoch++
	snapshot.Revision++
...
func rewriteStructuredHistoryTruth(
	...
	for index := range history {
		...
		if _, structured := compact.Carry(history[index].Text()); !structured {
			continue
		}
		_, found, err := compact.ParseTruthCapsule(history[index].Text())
		if err != nil {
		...
		rendered, err := compact.RenderStructured(
			compact.Summary{Window: removedMessages},
			truth,
			compact.Narrative{},
			0,
		)
...
func nextWindow(
	previous contextstore.WindowLedger,
	epoch uint64,
	revision uint64,
	...
	next, err := contextstore.NewWindowLedger(id, number)
	if err != nil {
		return contextstore.WindowLedger{}
	}
	return next
}
