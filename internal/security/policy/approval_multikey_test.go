...
func TestApprovalCacheMultiResourceKeysCoverSubsets(t *testing.T) {
	...
	first := Invocation{
		...
		Resources: []tool.Resource{a, b}, Capability: CapabilityWrite, Validated: true,
	}
	...
	subset := Invocation{
		...
		Resources: []tool.Resource{a}, Capability: CapabilityWrite, Validated: true,
	}
	if !cache.MatchInvocation(subset, now) {
		t.Fatal("approved path subset should skip ask")
	}
	...
	partial := Invocation{
		...
		Resources: []tool.Resource{a, c}, Capability: CapabilityWrite, Validated: true,
	}
