...
| Checkpoint | An immutable Session history/Profile artifact used for state-only restore or fork. | Workflow execution uses WorkGraph facts, not a separate checkpoint state machine. |
...
| Effect Owner | Extension/source/Plan/generation identity attached to a live process, connection, registration, subscription, lease, or timer. | Disable and revoke use it to drain or fence effects. |
| Fleet | A read/audit projection over WorkGraph state and ordered facts. | Fleet does not enqueue, claim, settle, or resume work. |
...
| Lane | Durable placement plus an explicit inline or tmux process adapter. | Placement is not lifecycle or scheduling authority. |
...
| Observation Envelope | A versioned, privacy-admitted causal evidence record with stable identity, correlations, bounded summary, and optional CAS payload reference. | It is not a Runtime Event or execution authority. |
...
| Terminal Measurement Snapshot | The digested Usage and latency fact frozen once at terminal convergence. | Receipt, Trace, and Terminal Envelope share it. |
...
| WorkGraph | The authoritative durable Run/Node/Attempt/Lease/Effect state machine committed as snapshots, ordered facts, receipts, and outbox rows. | Worker is the Claim authority; projections are read-only. |
| Workflow | A validated multi-step graph compiled into one durable WorkGraph. | Distinguish from a single Agent loop and Session Checkpoint. |
